...
var arena_routes_exports = {};
__export(arena_routes_exports, {
  default: () => arena_routes_default
});
module.exports = __toCommonJS(arena_routes_exports);
...
var isAdmin = (req, res, next) => __async(void 0, null, function* () {
  ...
  const user = yield getById2(req.id);
  if (!user) {
    return res.status(401).json({ error: "Unauthorized" });
  }
